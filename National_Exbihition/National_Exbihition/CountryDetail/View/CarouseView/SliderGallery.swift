...
class SliderGalleryController: UIViewController,UIScrollViewDelegate{
    ...
    func configureScrollerView(){
        self.view.frame = CGRect(x: 0,y: 0,
                                 width: self.scrollerViewWidth!, height: 400)
    ...
    func configurePlaceholder(){
        ...
        if #available(iOS 8.2, *) {
            ...
            placeholderImage = #imageLiteral(resourceName: "baccc")
        } else {
    ...
    func resetImageViewSource() {
        ...
        if self.currentIndex == 0 {
            self.leftImageView?.sd_setImage(with: URL.init(string: self.dataSource!.last!) , placeholderImage: placeholderImage,completed: nil)
               // .imageFromURL(self.dataSource!.last!,placeholder: placeholderImage)
            self.middleImageView?.sd_setImage(with: URL.init(string: self.dataSource!.first!) , placeholderImage: placeholderImage,completed: nil)
               // .imageFromURL(self.dataSource!.first!,placeholder: placeholderImage)
            let rightImageIndex = (self.dataSource?.count)!>1 ? 1 : 0 //保护
            self.rightImageView?.sd_setImage(with: URL.init(string: self.dataSource![rightImageIndex]) , placeholderImage: placeholderImage,completed: nil)
                //.imageFromURL(self.dataSource![rightImageIndex],
                  //                            placeholder: placeholderImage)
        }
            //当前显示的是最好一张图片
        else if self.currentIndex == (self.dataSource?.count)! - 1 {
            self.leftImageView?.sd_setImage(with: URL.init(string:self.dataSource![self.currentIndex-1]) , placeholderImage: placeholderImage,completed: nil)
//                .imageFromURL(self.dataSource![self.currentIndex-1],
//                                             placeholder: placeholderImage)
            self.middleImageView?.sd_setImage(with: URL.init(string: self.dataSource!.last!) , placeholderImage: placeholderImage,completed: nil)
            //.imageFromURL(self.dataSource!.last!,
                      //                         placeholder: placeholderImage)
            self.rightImageView?.sd_setImage(with: URL.init(string: self.dataSource!.first!) , placeholderImage: placeholderImage,completed: nil)
        }
            //其他情况
        else{
            self.leftImageView?.sd_setImage(with: URL.init(string:self.dataSource![self.currentIndex-1]) , placeholderImage: placeholderImage,completed: nil)
            self.middleImageView?.sd_setImage(with: URL.init(string:self.dataSource![self.currentIndex]) , placeholderImage: placeholderImage,completed: nil)
            self.rightImageView?.sd_setImage(with: URL.init(string:self.dataSource![self.currentIndex+1]) , placeholderImage: placeholderImage,completed: nil)
        }
    }
